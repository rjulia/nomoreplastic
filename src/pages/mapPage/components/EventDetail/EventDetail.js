  ...
  return (
    <div className='event-detail'>
      ...
      <div className="event-detail__boxtext">
        {
          event.category.map(category => (<Paragraph key={category} classN={`${category} event-detail__category`} text={category} />))
        }

        <Title tag="h2" text={event.title} />

        <Paragraph tag="p" text={moment(event.date).format('MM/DD/YYYY')} classN="event-detail__date" />
        <span className="event-detail__address"><FaMapMarkerAlt size="10" /> {event.place || ''}</span>

        <Paragraph classN="event-detail__description" text={event.content__en} />
        <Paragraph tag="a" classN="event-detail__link-web" href={event.webUrl || event.facebook} text={event.webUrl || event.facebook} />
        <Title tag="h3" classN={`${event.category} event-detail__subtitle `} text="Contant Details" />
        ...
        <a href={event.facebook} target="_blank" rel="noopener noreferrer" className="event-detail__facebook"><FaFacebookSquare size="20" /> </a>
      </div>
