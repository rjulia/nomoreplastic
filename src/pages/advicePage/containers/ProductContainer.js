...
const ProductContainer = ({ ids }) => {
  ...
  const productsInAdvice = data.getProducts.filter(product => ids.some(id => id.id === product.id))
  return (
    <div className="row">

      {productsInAdvice && productsInAdvice.map(product => <Product key={product.id} product={product} />)}
    </div>
  )
}
